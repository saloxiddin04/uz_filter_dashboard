  ...
  const getCountPrices = useCallback((inputData, i, requestData) => {
    ...
    timeoutId.current = setTimeout(async () => {
      ...
      try {
        ...
        inputData[i].selected_count = response?.devices_count_for_testing;
        inputData[i].price = response?.calculated_price
        setData([...inputData]);
      } catch (error) {
        console.log(error);
      }
  ...
  const handleChangeVal = (onChangeVal, i) => {
    ...
    if (onChangeVal?.target?.checked) {
      ...
      if (onChangeVal?.target?.name === 'certification_type') {
        dispatch(getCertificationTariff(onChangeVal?.target.value))
